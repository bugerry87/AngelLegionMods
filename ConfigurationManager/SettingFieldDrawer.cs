...
namespace ConfigurationManager
{
    ...
    {
        private static readonly IEnumerable<KeyCode> _keysToCheck = UnityInput.Current.SupportedKeyCodes.Except(new[] { KeyCode.Mouse0, KeyCode.None }).ToArray();
        ...
        private void DrawUnknownField(SettingEntryBase setting, int rightColumnWidth)
        {
            ...
            if (setting.ObjToStr != null && setting.StrToObj != null)
            {
                ...
                var result = GUILayout.TextField(text, ConfigurationManager.textStyle, GUILayout.MaxWidth(rightColumnWidth));
                if (result != text)
            ...
            else
            {
                ...
                {
                    var result = GUILayout.TextField(value, ConfigurationManager.textStyle, GUILayout.MaxWidth(rightColumnWidth));
                    if (result != value)
                        setting.Set(Convert.ChangeType(result, setting.SettingType, CultureInfo.InvariantCulture));
                }
                else
                {
                    GUILayout.TextArea(value, ConfigurationManager.textStyle, GUILayout.MaxWidth(rightColumnWidth));
                }
            }
